...
import { PlusCircle, ExternalLink, MicVocal, Bookmark, BookmarkCheck, Link as LinkIcon } from "lucide-react"; 
...
interface ContentItemCardProps {
  ...
  animationIndex?: number; // Changed from animationDelay to animationIndex
}
...
export function ContentItemCard({
  ...
  podcastFrequency,
  podcastTopics,
  animationIndex, // Default removed, should be handled by caller or CSS
  ...
  return (
    <div 
      className={cn(
        ...
        animationIndex !== undefined && `card-animate-delay-${animationIndex}` // Apply delay class
      )}
      // Removed inline style for animationDelay
    >
      <Card className={cn(
        ...
        <CardFooter className="p-4 pt-2 border-t mt-auto flex flex-col space-y-2">
          ...
          {(typeBadge === "Author" || typeBadge === "Fun Fact" || typeBadge === "Science Fact" || typeBadge === "Free Tool" || typeBadge === "Paid Tool" || typeBadge === "Newsletter" || typeBadge === "Podcast") && (
            <Button
              ...
              className={cn("w-full select-button", isImported ? "selected" : "")}
