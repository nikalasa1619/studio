...
const FetchPodcastsOutputSchema = z.object({
  podcasts: z
    ...
    // .min(3) // LLM might generate up to 5, then we filter
    // .max(5)
    .describe('A list of relevant podcast episodes or series with their details and relevance scores. Only includes podcasts with valid, working links.'),
});
...
Based on the topic "{{topic}}", find 3-5 relevant podcast episodes or series.
...
3. Podcast Link (A direct URL to the episode if possible, otherwise to the main podcast page. CRITICAL: Must be a complete and valid URL string starting with http or https. Prioritize well-known and stable websites that are unlikely to return a 404 error.)
