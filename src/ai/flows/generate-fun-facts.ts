...
const generateFunFactsFlow = ai.defineFlow(
  ...
  async (input: GenerateFunFactsInput): Promise<GenerateFunFactsOutput> => {
    ...
    if (!llmResponse.output) {
      console.log("No facts generated by LLM or output is missing.");
      return { funFacts: [], scienceFacts: [] };
    }
    ...
    const validatedFunFacts = llmResponse.output.funFacts ? await validateFactLinks(llmResponse.output.funFacts) : [];
    const validatedScienceFacts = llmResponse.output.scienceFacts ? await validateFactLinks(llmResponse.output.scienceFacts) : [];
